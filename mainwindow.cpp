    ...
    , ui(new Ui::MainWindow)
{
    ...
    m_stdinNotifier->setEnabled(false); // 默认禁用，只在需要时开启
    ...
    this->setStyleSheet(R"(
        ...
        QPushButton {
            ...
            padding: 8px;
            border-radius: 8px; /* <-- 增加圆角，让按钮也更圆润 */
            outline: none;      /* <-- 核心：移除焦点时的虚线框 */
        }
        QPushButton:hover {
...
void MainWindow::handleTerminalInput()
{
    ...
    if (!name.isEmpty()) {
        // [FIXED] 使用 .noquote() 来移除自动添加的引号
        qInfo().noquote() << "[OK] Name received:" << name << ". Starting registration process on the device...";

        // 将注册请求发送给工作线程
        QMetaObject::invokeMethod(m_processor, "startRegistration", Qt::QueuedConnection,
                                  Q_ARG(QString, name));
    } else {
